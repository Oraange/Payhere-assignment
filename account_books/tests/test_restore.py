...
class DeleteAccountBookViewTest(TestCase):
    ...
    def test_delete_account_book_success(self):
        ...
        response = self.client.patch('/account-books/trash/restore/1', content_type="application/json", **header)
        self.assertEqual(response.json(), {"message": "PUBLISHED"})
        self.assertEqual(response.status_code, 200)

    def test_delete_account_book_not_found(self):
        header = {'HTTP_Authorization': self.access_token_1}
        response = self.client.patch('/account-books/trash/restore/10', content_type="application/json", **header)
